...
export function CompleteSessionDialog({
  ...
  const [actualDistance, setActualDistance] = useState<number>(
    session.distance != null ? Math.round(displayDistance(session.distance, distanceUnit)) : 0
  )
  const [actualDuration, setActualDuration] = useState<number>(
    session.durationMinutes ? Math.round(session.durationMinutes / 5) * 5 : 0
  )
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reset form
      setActualDistance(session.distance != null ? Math.round(displayDistance(session.distance, distanceUnit)) : 0)
      setActualDuration(session.durationMinutes ? Math.round(session.durationMinutes / 5) * 5 : 0)
      setRpe(0)
      setUserNotes('')
  ...
  if (showConfirmation) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-hidden flex flex-col bg-card">
                ...
                <>
                  <div className="space-y-2">
                    ...
                    {session.distance && (
                      <p className="text-xs text-[#696863]">
                        Planned: {Math.round(displayDistance(session.distance, distanceUnit))} {distanceUnit}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="rpe" className="text-sm font-normal text-foreground">
                      What was your average RPE this session?
                    </Label>
                    <NumberInput
                      id="rpe"
                      value={rpe}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 border border-input rounded-md bg-background min-h-[80px] text-sm font-normal"
                    />
                  </div>
                </>
              )}
