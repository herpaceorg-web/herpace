...
export function HormoneWaveBackground({ opacity = 0.15 }: HormoneWaveBackgroundProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-80 overflow-hidden pointer-events-none" style={{ zIndex: 0 }}>
      <svg
      ...
      >
        ...
        <path
          fill={`${HORMONE_COLORS.estrogen}${opacityHex}`}
        ...
        <path
          fill={`${HORMONE_COLORS.progesterone}${opacityHex}`}
        ...
        <path
          fill={`${HORMONE_COLORS.fsh}${opacityHex}`}
        ...
        {/* LH wave - blue */}
        <path
          fill={`${HORMONE_COLORS.lh}${opacityHex}`}
