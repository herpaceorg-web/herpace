...
public class CycleController : ControllerBase
{
    ...
    /// <summary>
    /// Reports a period start date and triggers plan regeneration if needed.
    /// Returns 400 if the date is in the future.
    /// </summary>
    [HttpPost("report")]
    public async Task<IActionResult> ReportPeriod([FromBody] ReportPeriodRequest request)
    {
        ...
        _logger.LogInformation("Reporting period for runner {RunnerId}, date {Date}", runnerId, request.PeriodStartDate);

        if (request.PeriodStartDate > DateTime.UtcNow)
        {
        ...
        try
        {
            var response = await _cycleTrackingService.ReportPeriodStartAsync(runnerId, request.PeriodStartDate);
            return Ok(response);
        }
